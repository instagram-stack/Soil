...
>
	...
	<div
		class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-[#E6FF5A] opacity-[0.02] blur-[200px] rounded-full pointer-events-none"
	></div>
	...
	>
		...
		>
			{#each services as service, i}
				...
				>
					...
					<div
						class="absolute -top-32 -left-32 w-96 h-96 bg-[#E6FF5A] opacity-[0.03] group-hover:opacity-[0.12] blur-[100px] rounded-full transition-opacity duration-700 pointer-events-none z-0"
					></div>
					...
					<!-- Icon -->
					<div
						class="absolute top-8 right-8 z-20 text-white/20 group-hover:text-[#E6FF5A] transition-colors duration-700 transform group-hover:scale-110"
					>
						<svg
							class="w-8 h-8"
							fill="none"
		...
		<div class="mt-16 flex justify-center w-full">
			<a
				href="/services"
				class="px-8 py-3.5 rounded-full border border-white/20 text-white/80 hover:bg-[#E6FF5A] hover:text-black hover:border-[#E6FF5A] transition-all duration-300 font-sans cursor-pointer text-sm font-medium flex items-center gap-2 group"
			>
				<span>Explore Full Disciplines & Capabilities</span>
